...
--
-- CAPSULE 16 ADDITION:
--   - marketPrice(itemID) → cheapest unit price from listings
--   - Returns price (copper) + source ("ah" or "manual")
...
-------------------------------------------------
-- Market price: cheapest unit price from listings
-------------------------------------------------
-- Returns the cheapest per-unit price from AH listings,
-- falling back to manual price if no listings exist.
-- Returns: price (copper) or nil

function Quote.marketPrice(itemID)
    local listings = ns.Listings.getListings(itemID)
    if listings and #listings > 0 then
        local bestUnitPrice = math.huge
        for _, l in ipairs(listings) do
            if l.count > 0 then
                local unitPrice = l.buyout / l.count
                if unitPrice < bestUnitPrice then
                    bestUnitPrice = unitPrice
                end
            end
        end
        if bestUnitPrice ~= math.huge then
            return math.floor(bestUnitPrice), "ah"
        end
    end

    -- Fallback to manual price
    local manual = ns.Prices.get(itemID)
    if manual then
