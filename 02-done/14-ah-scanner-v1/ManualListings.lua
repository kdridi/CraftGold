...
-- /cg scan <itemID>
-- /cg scan cancel — cancel a stuck scan
-- /cg scan (no args) — show scan status
cmd:register {
    name = "scan",
    ...
    handler = function(a)
        if ns.Scanner.isActive() then
            local scanID = ns.Scanner.getTargetItemID()
            local scanName = ns.ItemInfo.formatName(scanID)
            printMsg(string.format(
                "|cFF4FC3F7[Scanner]|r Scan in progress for %s (%d). Use |cFFFFFF00/cg scan cancel|r to abort.",
                scanName, scanID))
            return
        end

        local name = ns.ItemInfo.formatName(a.itemID)
        printMsg(string.format("|cFF4FC3F7[Scanner]|r Scanning AH for %s (%d)...", name, a.itemID))

        local ok, err = ns.Scanner.scan(a.itemID, function(results, skipped)
            -- Handle cancellation
            if skipped and skipped.cancelled then
                ...
                printMsg(string.format(
                    "|cFFFF0000[Scanner]|r Scan for %s (%d) cancelled (AH closed?)",
            ...
            if #results == 0 then
                printMsg(string.format(
                    "|cFF4FC3F7[Scanner]|r %s (%d) — no buyout listings found",
                    name, a.itemID))
            else
                ...
                printMsg(string.format(
                    "|cFF4FC3F7[Scanner]|r %s (%d) — |cFF00FF00%d listings|r found (%d total, %s)",
                    name, a.itemID, #results, totalQty,
                    ns.Money.formatColored(totalCost)))

                -- Show skipped stats if any
    ...
    subs = {
        cancel = {
            help = "Cancel a stuck scan",
            handler = function()
                local cancelled = ns.Scanner.cancel()
                if cancelled then
                    printMsg("|cFF4FC3F7[Scanner]|r Scan cancelled.")
                else
