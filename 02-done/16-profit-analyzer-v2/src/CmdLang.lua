    ...
    while node.subs do
        ...
        if not tok then
            ...
            if node.handler then
                break  -- Hybrid node: treat as leaf (has own handler + args)
            end
            local names = {}
        ...
        if not node.subs[tok.text] then
            -- Token doesn't match any subcommand
            if node.handler then
                break  -- Hybrid node: treat as leaf
            end
            local names = {}
            for k in pairs(node.subs) do names[#names + 1] = k end
            table.sort(names)
...
local function generateHelp(self, showAll)
    ...
    local function walkTree(node, depth)
        ...
        if node.subs then
            -- Branch node (may also be hybrid with handler + args)
            ...
            -- If hybrid (has handler + args), show the leaf usage too
            if node.handler and node.args then
...
function CmdLang:register(def)
    ...
    local existing = self.commands[def.name]
    if existing then
        -- Merge: handler/args from either registration, subs merged together
        local merged = {
            name      = def.name,
            help      = def.help or existing.help,
            handler   = def.handler or existing.handler,
            args      = def.args or existing.args,
            subs      = {},
            path      = def.name,
            condition = def.condition or existing.condition,
        }

        -- Copy existing subs
        if existing.subs then
            for k, v in pairs(existing.subs) do
                merged.subs[k] = v
            end
        end

        -- Copy/add new subs (overwrite if same name)
        if def.subs then
            for k, v in pairs(def.subs) do
                merged.subs[k] = compileNode(v, self.types, def.name .. " " .. k)
            end
        end

        -- Clear subs if empty
        if not next(merged.subs) then merged.subs = nil end

        self.commands[def.name] = merged
    else
