...
--
-- CAPSULE 16 REFACTOR (Profit Analyzer v2):
--   - analyze() uses Quote.marketPrice() instead of Prices.get()
--   - Applies 5% AH commission (AH_CUT constant)
--   - Tracks price source ("ah" or "manual") for transparency
--   - Result includes netSell, ahCut, priceSource fields
--
-- Algorithm:
...
-- Constants
-------------------------------------------------
local AH_CUT = 0.05  -- 5% auction house commission

-- Returns AH cut percentage for display purposes
function Calculator._getAhCutPercent()
    return AH_CUT * 100
end
...
-------------------------------------------------
-- CAPSULE 16 REFACTOR (Profit Analyzer v2):
--   - Uses Quote.marketPrice() instead of Prices.get()
--   - Applies AH commission (5%) to estimate net revenue
--   - Tracks price source ("ah" or "manual") for transparency
--   - profit = netSellPrice - craftCost (after commission)
--
...
function Calculator.analyze()
    ...
    for _, recipe in pairs(ns.DB.recipes) do
        ...
        -- Get market-based sell price estimate
        local sellPrice, priceSource = ns.Quote.marketPrice(outputID)

        if sellPrice and sellPrice > 0 then
            local costResult = Calculator._calculate(outputID, 1, state)
            if costResult and costResult.cost > 0 then
                local netSell   = math.floor(sellPrice * (1 - AH_CUT))
                local ahCut     = sellPrice - netSell
                local profit    = netSell - costResult.cost
